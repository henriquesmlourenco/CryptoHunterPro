...
// checkLicense verifica a licenca antes de iniciar o programa
func checkLicense() {
	// Se nao tem data de expiracao, e vitalicio - passa direto
	if expirationDate == "" {
		return
	}
	...
	if err != nil {
		...
		fmt.Println()
		fmt.Println("  Pressione Enter para sair / Press Enter to exit...")
		fmt.Scanln()
		os.Exit(1)
	}

	// Parsear data de expiracao
	...
	// Definir expiracao no final do dia (23:59:59)
	expDate = expDate.Add(23*time.Hour + 59*time.Minute + 59*time.Second)
	...
	if verifiedTime.After(expDate) {
		...
		fmt.Println("================================================================")
		fmt.Println()
		fmt.Printf("  Data de expiracao / Expiration date: %s/%s/%s\n",
			expirationDate[6:8], expirationDate[4:6], expirationDate[0:4])
	...
	fmt.Printf("  [OK] Licenca valida - expira em %d dia(s)\n", days)
	fmt.Printf("  [OK] License valid - expires in %d day(s)\n", days)
	fmt.Println()
}
