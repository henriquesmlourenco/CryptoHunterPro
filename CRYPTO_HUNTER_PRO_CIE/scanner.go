			...
			for _, dp := range ng.Derivations {
				...
				case "bch":
					...
					case "near":
						...
						time.Sleep(500 * time.Millisecond)
					case "liquid":
						liquidResult, err := CheckLiquidBalance(address)
						if err != nil {
							fmt.Printf("       [%d] %s -> ERRO: %s\n", idx, address, err.Error())
						} else if liquidResult.HasBalance {
							result.NativeBalance = liquidResult.NativeBalance
							result.NativeSymbol = liquidResult.NativeSymbol
							result.HasBalance = true
							result.HasHistory = liquidResult.HasHistory
							result.TxCount = liquidResult.TxCount
							result.Tokens = liquidResult.Tokens
							allResults = append(allResults, result)
							totalWithBalance++
							derivationBalance++
							fmt.Printf("       [%d] %s -> SALDO: %s L-BTC", idx, address, liquidResult.NativeBalance)
							for _, tok := range liquidResult.Tokens {
								fmt.Printf(" | %s: %s", tok.Symbol, tok.Balance)
							}
							fmt.Println()
						} else if liquidResult.HasHistory {
							result.NativeBalance = liquidResult.NativeBalance
							result.NativeSymbol = liquidResult.NativeSymbol
							result.HasHistory = true
							result.TxCount = liquidResult.TxCount
