...
func showProgramInfo() {
	...
	if uiLanguage == "pt" {
		...
		fmt.Println("  - Busca de saldo e tokens em Liquid Network (L-BTC + USDt + descoberta automatica)")
		fmt.Println("  - Busca de saldo em Litecoin (LTC) e Dogecoin (DOGE)")
		fmt.Println("  - Suporte a seeds Electrum (Standard, Segwit, 2FA) - BTC e LTC")
		...
		fmt.Println()
		fmt.Println("  ============================================================")
		fmt.Println("  REDES SUPORTADAS E DERIVACOES (29 blockchains):")
		fmt.Println("  ============================================================")
		fmt.Println()
		...
		fmt.Println("  Liquid Network (L-BTC)  m/44'/1776'/0'/0/x      ex1...")
		...
		fmt.Println("  Liquid Network (L-BTC): Saldo nativo (L-BTC) + tokens (USDt, LCAD, etc)")
		fmt.Println("    + Descoberta automatica de qualquer Liquid Asset via Blockstream API")
		...
		fmt.Println("  - Liquid Network (L-BTC + USDt + automatic token discovery) balance search")
		...
		fmt.Println("  - Electrum seed support (Standard, Segwit, 2FA) - BTC and LTC")
		...
		fmt.Println()
		fmt.Println("  ============================================================")
		fmt.Println("  SUPPORTED NETWORKS AND DERIVATIONS (29 blockchains):")
		fmt.Println("  ============================================================")
		fmt.Println()
		...
		fmt.Println("  Liquid Network (L-BTC): Native balance (L-BTC) + tokens (USDt, LCAD, etc)")
		fmt.Println("    + Automatic discovery of any Liquid Asset via Blockstream API")
...
// ============================================================================
// FORMATO DA SEED (BIP-39 ou ELECTRUM)
// ============================================================================

func chooseSeedFormat() string {
	for {
		fmt.Println()
		fmt.Println("================================================================")
		fmt.Println("  " + t("FORMATO DA SEED", "SEED FORMAT"))
		fmt.Println("================================================================")
		fmt.Println()
		fmt.Println(t(
			"  [1] BIP-39 (Padrao - compativel com todas as redes)",
			"  [1] BIP-39 (Standard - compatible with all networks)"))
		fmt.Println(t(
			"      - Usado pela maioria das carteiras (Ledger, Trezor, MetaMask, etc)",
			"      - Used by most wallets (Ledger, Trezor, MetaMask, etc)"))
		fmt.Println(t(
			"      - Busca em todas as 29 blockchains suportadas",
			"      - Searches across all 29 supported blockchains"))
		fmt.Println()
		fmt.Println(t(
			"  [2] Electrum (Carteira Electrum - apenas BTC e LTC)",
			"  [2] Electrum (Electrum Wallet - BTC and LTC only)"))
		fmt.Println(t(
			"      - Formato exclusivo da carteira Electrum",
			"      - Exclusive format of the Electrum wallet"))
		fmt.Println(t(
			"      - Usa derivacao diferente do BIP-39 (salt 'electrum')",
			"      - Uses different derivation from BIP-39 (salt 'electrum')"))
		fmt.Println(t(
			"      - Detecta automaticamente: Standard (1...), Segwit (bc1q...), 2FA",
			"      - Auto-detects: Standard (1...), Segwit (bc1q...), 2FA"))
		fmt.Println(t(
			"      - Busca em BTC e LTC",
			"      - Searches BTC and LTC"))
		fmt.Println()
...
// getSeedManualElectrum le uma seed Electrum sem validacao BIP-39
func getSeedManualElectrum() string {
	for {
		fmt.Println()
		fmt.Println("================================================================")
		fmt.Println("  " + t("DIGITE A SEED ELECTRUM", "ENTER THE ELECTRUM SEED"))
		fmt.Println("================================================================")
		fmt.Println()
		fmt.Println(t(
			"  Digite ou cole as palavras da seed Electrum (12 ou 24 palavras):",
			"  Type or paste the Electrum seed words (12 or 24 words):"))
		fmt.Println(t(
			"  NOTA: Seeds Electrum NAO passam na validacao BIP-39.",
			"  NOTE: Electrum seeds do NOT pass BIP-39 validation."))
		fmt.Println(t(
			"  O programa detectara automaticamente o tipo (Standard/Segwit/2FA).",
			"  The program will auto-detect the type (Standard/Segwit/2FA)."))
		fmt.Println(t(
		...
		if wordCount != 12 && wordCount != 24 {
			fmt.Println()
			fmt.Printf(t(
				"  [!] ERRO: Seed com %d palavras. Seeds Electrum devem ter 12 ou 24 palavras.\n",
				"  [!] ERROR: Seed with %d words. Electrum seeds must have 12 or 24 words.\n"), wordCount)
			continue
		}
		...
		if seedType != "" {
			typeName := map[string]string{
				"standard": t("Standard (P2PKH - enderecos 1...)", "Standard (P2PKH - addresses 1...)"),
				"segwit":   t("Segwit (P2WPKH - enderecos bc1q...)", "Segwit (P2WPKH - addresses bc1q...)"),
				"2fa":      t("2FA (Multisig)", "2FA (Multisig)"),
			}
			fmt.Println()
			fmt.Printf(t(
				"  [OK] Seed Electrum valida! Tipo: %s\n",
				"  [OK] Valid Electrum seed! Type: %s\n"), typeName[seedType])
		} else {
			fmt.Println()
			fmt.Println(t(
				"  [!] AVISO: Seed nao reconhecida como Electrum valida.",
				"  [!] WARNING: Seed not recognized as valid Electrum."))
			fmt.Println(t(
				"  Sera tratada como Standard (m/0/x). Deseja continuar? (S/N)",
				"  Will be treated as Standard (m/0/x). Continue? (Y/N)"))
			confirm := getUserInput("  > ")
			if strings.ToLower(confirm) != "s" && strings.ToLower(confirm) != "y" {
				continue
			}
		}
...
func configureAPIKeys() {
	...
	fmt.Println(t(
		"     TON, Zcash, XRP, Stellar, Algorand, Sui, Near, Liquid",
		"     TON, Zcash, XRP, Stellar, Algorand, Sui, Near, Liquid"))
	fmt.Println()
