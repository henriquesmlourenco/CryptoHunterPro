...
func runManualSeedFlow() {
	// Perguntar formato da seed
	seedFormat := chooseSeedFormat()

	// Validação (apenas para BIP-39)
	...
	// Input da seed
	var seed string
	if seedFormat == "electrum" {
		seed = getSeedManualElectrum()
	} else {
	...
	if seedFormat == "electrum" {
		// Fluxo Electrum: apenas BTC + LTC
		config := &ScanConfig{
			Seeds:      []string{seed},
			SeedSource: t("Manual (Electrum)", "Manual (Electrum)"),
			SeedFormat: "electrum",
			StartIndex: startIdx,
			EndIndex:   endIdx,
		...
		// Executar scan Electrum
		results := runElectrumScan(config)
