...
// Path: m/44'/607'/account' (SLIP-0010 Ed25519)
// Generates real TON wallet V4R2 address (UQ... format)
func DeriveTONKeypair(seedPhrase string, passphrase string, index int) (string, string, error) {
	...
	// Generate real TON wallet V4R2 address (UQ... user-friendly format)
	address, err := tonWalletV4R2Address([]byte(pubKey))
	if err != nil {
		return "", "", fmt.Errorf("erro ao gerar endereco TON: %v", err)
	}
...
// ============================================================================
// GERACAO DE ENDERECOS - LIQUID NETWORK (L-BTC)
// ============================================================================

// deriveLiquidAddress gera endereco Liquid Network (ex1... bech32 native segwit)
// Liquid usa HRP "ex" ao inves de "bc" do Bitcoin
func deriveLiquidAddress(privKey []byte) (string, string) {
	pubKey := privateKeyToCompressedPubKey(privKey)
	hash160 := hash160Bytes(pubKey)
	// Liquid usa bech32 com HRP "ex" (native segwit witness v0)
	conv, _ := bech32ConvertBits(hash160, 8, 5, true)
	addr, _ := bech32Encode("ex", 0x00, conv)
	wif, _ := btcutil.NewWIF(func() *btcec.PrivateKey {
		pk, _ := btcec.PrivKeyFromBytes(privKey)
		return pk
	}(), &chaincfg.MainNetParams, true)
...
// bech32ConvertBits converte entre diferentes tamanhos de bits
...
func DeriveAddress(seedPhrase string, passphrase string, dp DerivationPath, index int) (string, string, error) {
	...
	case "liquid":
		address, privateKeyStr = deriveLiquidAddress(privKey)
	default:
